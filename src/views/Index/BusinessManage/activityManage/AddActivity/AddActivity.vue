                        ...
                        <el-table
                            ...
                            <el-table-column align="center" label="票种信息">
                                <template slot-scope="scope">
                                    {{scope.row.ticketRefundType  ? "付费票" : "免费票"}}
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column align="center" property="address" label="价格">
                                <template slot-scope="scope">
                                    <el-input-number v-model="scope.row.ticketPrice" :min="0" :controls="false" :size="'small'"></el-input-number>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column align="center" label="退款范围">
                                <template slot-scope="scope">
                                    <el-select v-model="scope.row.refundType">
                                        <el-option :value="0" label="无条件退款"></el-option>
                                        <el-option :value="1" label="不能退款"></el-option>
                                    </el-select>
                                </template>
                            ...
                            <div class="no-force-item-box" v-for="(formItem, index) in formSettingDtoList" :key="index">
                                ...
                                <div class="no-force-item-content">
                                    ...
                                    <div class="select-item-box" v-else-if="formItem.type === 2 || formItem.type === 3">
                                        ...
                                        <el-button class="definite" @click="addSelectItem(formItem.content, formItem.inputCache, formItem)" style
                                        ="color:white;background:#ff7000;margin-left:5px;">确定</el-button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script lang="ts">
    ...
    export default class AddActivity extends Vue {
        ...
        saveAsDraft() {
            ...
            this.$request.post("/merchant/activity/add_activity", {
                accountId : Number(localStorage.accountId),
                ticketList : this.addTicketTableData.map((rua : any, index) => {
                    return {
                        ...rua,
                        sequence : index++
                    }
        ...
        public addActivity() {
            ...
            this.$http.post("/merchant/activity/add_activity", {
                accountId : Number(localStorage.accountId),
                ticketList : this.addTicketTableData.map((rua : any, index) => {
                    return {
                        ...rua,
        ...
        mounted() {
            ...
            // setTimeout(() => {
            //     let addRange = this.editorRef.quill.getSelection();
            //     this.editorRef.quill.insertEmbed(addRange !== null ? addRange.index : 0, 'image', 'https://www.sakura-snow.com/wp-content/themes/kratos/static/images/avatar/avatar.jpg');
            // }, 5000)
        }
    }
</script>
